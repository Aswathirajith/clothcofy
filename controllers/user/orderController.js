...
const placeOrder = async (req, res) => {
    try {
        ...
        if(totalAmount>1000 && paymentMode === "COD" )
        {
    ...
    try {
        ...
        if (orderData.products && orderData.products.length > 0) {
         ...
         if(product.status === "Return Requested")  
            {
                res.json({ message: 'Return request already submitted' });
            } 
...
const generateInvoice = async (req, res) => {
    try {
        ...
        orderData.products.forEach((product) => {
            if(product.status !== 'Order Cancelled' && product.status !== 'Cancelled by clothyFy' && product.status === 'Order Delivered'){
                products.push({
                    quantity: product.quantity ,
