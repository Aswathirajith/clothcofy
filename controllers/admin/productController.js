...
const addProduct = async (req, res) => {
    try {
        ...
        if (!prdctName || /^\s*$/.test(prdctName)) {
            const prdctData = await Product.find({});
            return res.render("admin/addProduct", { prdctData, cateData, message: "Enter a valid product name" });
        }

        // Checking valid category description / space check
        if (!prdctDescription || /^\s*$/.test(prdctDescription)) {
            const prdctData = await Product.find({});
            return res.render("admin/addProduct", { prdctData, cateData, message: "Enter a valid product description" });
        }
        ...
        if (parsedPrdctPrice <= 0) {
            const prdctData = await Product.find({});
            return res.render("admin/addProduct", { prdctData, cateData, message: "Price of the product should be greater than zero" });
        }

        // Checking the quantity should be at least one
        if (parsedPrdctQuantity < 1) {
            const prdctData = await Product.find({});
            return res.render("admin/addProduct", { prdctData, cateData, message: "Quantity of the product should be at least one" });
        }

        // Checking if the image files are available
        if (!imgFiles || imgFiles.length === 0 || imgFiles.length < 4) {
            const prdctData = await Product.find({});
            return res.render("admin/addProduct", { prdctData, cateData, message: "Please enter at least 4 images" });
        }
...
const editProduct = async (req, res) => {
    try {
        ...
        if (prdctData) {
            res.render("admin/editProduct", { prdctData, cateData, productImagesArray });
        }
        else {
            res.redirect("/admin/productsList");
        }
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (!prdctData) {
             res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Product not found" });
        }

        // checking valid product name / space check
        if (!prdctName || /^\s*$/.test(prdctName)) {
            return res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Enter a valid product name" });
        }

        // checking valid category description / space check
        if (!prdctDescription || /^\s*$/.test(prdctDescription)) {
            return res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Enter a valid product description" });
        }
        ...
        // checking the price should not be less than 0
        if (parsedPrdctPrice <= 0) {
            return res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Price of the product should be greater than zero" });
        }

        // checking the quantity should be at least one
        if (parsedPrdctQuantity < 1) {
            return res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Quantity of the product should be at least one" });
        }
        ...
        if (updatedProduct) {
            ...
            return res.render("admin/editProduct",{prdctData, productImagesArray, success: false, message: "Failed to update product" });
        }
