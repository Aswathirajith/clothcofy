...
const addCategory = async (req, res) => {
    ...
    try {
        ...
        if (!cateName || /^\s*$/.test(cateName)) {
            const cateData = await Category.find({});
            return res.render("admin/addCategory", { cateData, message: "Enter a valid name" });
        }
        ...
        if (existingCategory) {
            const cateData = await Category.find({});
            return res.render("admin/addCategory", { cateData, message: "Category already exist.!" });
        }

        // checking valid category description
        if (!cateDescription || /^\s*$/.test(cateDescription)) {
            const cateData = await Category.find({});
            return res.render("admin/addCategory", { cateData, message: "Enter a valid category" });
        }
        ...
        if (cateData) {
            const cateData = await Category.find({});
            res.render("admin/addCategory", { cateData, message: "Category added successfully" });
        }
...
const editCategory = async (req, res) => {
    try {
        ...
        if (cateData) {
            res.render("admin/editCategory", { cateData });
        }
        else {
            res.redirect("/admin/addCategory");
        }
...
const updateCategory = async (req, res) => {
    try {
        ...
        // checking valid category name
        if (!cateName || /^\s*$/.test(cateName)) {
            return res.render("admin/editCategory", { cateData, message: "Enter a valid name" });
        }
        ...
        if (existingCategory && existingCategory._id.toString() !== cateId) {
            return res.render("admin/editCategory", { cateData, message: "Category already exist.!" });
        }

        // checking valid category description
        if (!cateDescription || /^\s*$/.test(cateDescription)) {
            const cateData = await Category.find({});
            return res.render("admin/editCategory", { cateData, message: "Enter a valid category" });
        }
